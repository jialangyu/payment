package com.payment.base.dao;


import com.payment.base.pojo.PayInfo;
import com.payment.base.pojo.PayMoney;
...
public interface PayMoneyDao extends JpaRepository<PayMoney,String>,JpaSpecificationExecutor<PayMoney> {
    ...
    @Query("select new com.payment.base.pojo.PayInfo(p.id ,t.typename ,u.username,p.paycount,p.createtime,p.remark ) from PayMoney p inner join Type t on p.typeid=t.id inner join User u on p.payuserid=u.id where p.stat=?1 order by p.createtime desc")
